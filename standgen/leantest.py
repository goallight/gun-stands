...
def build(spec: StandSpec, angles=None, label_size: float = 7.0):
    ...
    # The lean is sideways (around Z), so each tower's X footprint grows.
    max_lean = max(angles)
    lean_shift = h * np.sin(np.radians(max_lean))
    slot_w = w + lean_shift   # X footprint of the most leaned tower
    gap = 6.0
    total_x = gap + len(angles) * (slot_w + gap)
    total_z = d + 2 * wall + 10
    total_h = base_y + h
    ...
    for angle in angles:
        ...
        # Lean sideways (rotate about Z axis, top tilts to -X)
